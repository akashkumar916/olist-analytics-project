...
class TotalSpent extends React.Component {
  ...
  onSubmit(e) {
    ...
    const url = 'http://localhost:8080/categoryRevenuePercentage';
    const x = [];
    const y = [];
    axios.post(url,{"frequency":this.state.param1,"productCategory":this.state.param3, "startDate" : format(new Date(this.state.param4), 'dd-MMM-yy'), "endDate" : format(new Date(this.state.param5), 'dd-MMM-yy')})
    .then(response => {
