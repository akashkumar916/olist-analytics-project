...
class TotalSpent extends React.Component {
  ...
  onSubmit(e) {
    ...
    const url = 'http://localhost:8080/productPerformance';
    const x = [];
    const y = [];
    const state_id= [this.state.param2];
   // state_id.push(this.state.param2);
    console.log(state_id);
    axios.post(url,{"productCategory":this.state.param3,"frequency":this.state.param1,"states":state_id, "startDate" : format(new Date(this.state.param4), 'dd-MMM-yy'), "endDate" : format(new Date(this.state.param5), 'dd-MMM-yy')})
    .then(response => {
